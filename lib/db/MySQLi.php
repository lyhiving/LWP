...
class DB_MySQLi extends DBQuery {
    ...
    public function __construct($config) {
        if (!function_exists('mysqli_query')) {
            lwp_error(sprintf(__('您的 PHP 似乎缺少所需的 %s 扩展。'),'MySQLi'), LOGGER_FATAL);
        }
        if (!empty($config)) {
    ...
    public function connect(){
        ...
        if (mysqli_connect_errno()) {
            lwp_error(sprintf(__('数据库链接错误：%s'), mysqli_connect_error()), LOGGER_FATAL);
        }
        return $this->conn;
    }
    /**
    ...
    public function query($sql){
        ...
        if (!($result = $func($this->conn, $sql))) {
            if (in_array(mysqli_errno($this->conn),array(2006,2013)) && ($this->goneaway-- > 0)) {
                ...
                // 重置计数
                $this->goneaway = 3;
                lwp_error(sprintf(_("MySQL Query Error:%s"), $sql . "\r\n\t" . mysqli_error($this->conn)),LOGGER_ERROR);
            }
        }
