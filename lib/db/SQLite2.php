...
class DB_SQLite2 extends DBQuery {
    ...
    public function __construct($config) {
        if (!function_exists('sqlite_query')) {
            lwp_error(sprintf(__('您的 PHP 似乎缺少所需的 %s 扩展。'), 'SQLite2'),LOGGER_FATAL);
        }
        if (!empty($config)) {
    ...
    public function open($dbname, $mode=0666) {
        ...
        // 验证连接是否正确
        if (!$this->conn) {
            lwp_error(sprintf(__('数据库链接错误：%s'), $error),LOGGER_FATAL);
        }
        // 设置10秒等待
    ...
    public function query($sql){
        // 验证连接是否正确
        if (!$this->conn) {
            lwp_error(__('提供的参数不是一个有效的SQLite的链接资源。'),LOGGER_ERROR);
        }
        $args = func_get_args();
