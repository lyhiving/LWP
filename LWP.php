...
define('LWP_VER', '0.3');
// framework path
defined('LWP_PATH') or define('LWP_PATH', dirname(__FILE__));
// framework query field
defined('LWP_QFIELD') or define('LWP_QFIELD', 'q');
// check environment
...
// app path
defined('APP_PATH') or define('APP_PATH', LWP_PATH);
// app root
...
function get_config($name=null, $file='common') {
    ...
    if (!isset($configures[$file])) {
        ...
        $default = array(
            ...
            // app common autoload
            'app_autoload' => array(
                '^DB_(.+?)$' => LWP_PATH . '/lib/db/$1.php',
                '^Services_JSON$' => LWP_PATH . '/lib/JSON.php',
...
function load_config($file, $super=null) {
    $config = array();
    // 先加载 LWP_PATH 目录
    if (is_ifile(LWP_PATH . '/conf/' . $file . '.php')) {
        include LWP_PATH . '/conf/' . $file . '.php';
    }
    // 加载 APP_PATH 目录
...
final class App {
    ...
    public function __construct() {
        ...
        if (IS_CLI) {
            ...
            $this->rewrite = ($request_uri != PHP_FILE) && (!is_ifile(dirname(LWP_PATH) . $request_uri));
            $this->qfield  = isset($_GET[LWP_QFIELD]);
    ...
    public function __destruct() {
        ...
        if (IS_CLI) {
            ...
            printf("Powered-By: LWP/%s (lukin.net)\n", LWP_VER);
        ...
        // http mode
            ...
            if (!headers_sent()) {
                ...
                header('X-Powered-By: LWP/' . LWP_VER . ' (lukin.net)');
